...
const { matchResumeToJob } = require("../../services/geminiAnalysis");
...
// Regex-based keyword fallback used when both Gemini and the ML service fail.
function buildLocalMatch(mergedSkills, descriptionLower) {
...
function buildGeminiMatch(result, mergedSkills, resumeProfile, resumeProfileId) {
  return {
    ...result,
    resumeProfileId: resumeProfile ? String(resumeProfile._id) : (resumeProfileId || ""),
    studentSkills: mergedSkills,
    student_skills: mergedSkills,
    compatibility_score: result.matchScore,
    matched_skills: result.matchedSkills,
    missing_skills: result.missingSkills.map((item) => ({
      skill: item.skill,
      weight: item.importance === "high" ? 0.7 : item.importance === "low" ? 0.3 : 0.5
    })),
    summary: result.reasoning || "AI match from your resume against this job.",
    roadmap: [],
    allRoleScores: {},
    all_role_scores: {},
    targetRole: "",
    ...
    roleDescription: "",
    role_description: "",
    jobSkills: result.matchedSkills,
    job_skills: result.matchedSkills
  };
}
...
router.post("/", async (req, res) => {
  try {
    ...
    // 1. Try Gemini first; fall through if it fails or returns nothing usable.
    const geminiResult = await matchResumeToJob(mergedSkills, jobDescription);
    if (geminiResult) {
      return res.json(buildGeminiMatch(geminiResult, mergedSkills, resumeProfile, resumeProfileId));
    }

    // 2. Fall back to the existing ML-service call.
    ...
    return res.json(buildLocalMatch(mergedSkills, descriptionLower));
  }
});
